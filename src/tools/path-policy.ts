...
function walkSegments(currentBase: string, segments: string[], symlinkDepth: number): string {
  ...
  if (stats.isSymbolicLink()) {
    ...
    return resolveAbsolutePath(path.join(resolvedTarget, ...tail), symlinkDepth + 1);
  }
  ...
  return walkSegments(currentPath, tail, symlinkDepth);
}
